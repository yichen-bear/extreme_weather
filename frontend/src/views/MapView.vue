      ...
      <div class="map-area">
        <div class="map-stack" :key="currentActive">
          ...
          <map name="disaster-map">
            ...
            <area v-if="currentActive === 'wildfire'" alt="southEU1" title="southEU1" coords="874,212,879,197,861,189,852,188,839,193,824,197,829,209,836,218,834,229,820,231,797,231,796,242,794,256,797,268,804,272,813,274,834,271,844,247,855,237,863,228,876,228" shape="poly" @click.prevent="openModal('southEU1')">
            <area v-if="currentActive === 'wildfire'" alt="southEU2" title="southEU2" coords="882,211,894,208,906,208,902,216,915,233,923,241,931,249,922,259,917,269,909,254,904,262,906,238,894,224,890,216,886,238,893,261,875,221,877,216" shape="poly" @click.prevent="openModal('southEU2')">
            <area v-if="currentActive === 'wildfire'" alt="southEU3" title="southEU3" coords="1049,238,1024,244,992,239,984,246,979,233,968,234,950,242,941,249,942,258,949,266,955,273,963,258,963,244,977,254,981,262,987,268,1005,271,1021,271,1037,265,1049,266,1059,265,1068,263,1064,252,1057,241" shape="poly" @click.prevent="openModal('southEU3')">
            <area v-if="currentActive === 'storm'" alt="1eastAsiaTYPHOON" title="1eastAsiaTYPHOON" coords="1423,170,1413,177,1402,189,1404,197,1413,212,1389,234,1329,241,1321,228,1303,225,1262,194,1244,210,1243,220,1241,236,1231,252,1235,264,1254,303,1294,322,1337,317,1335,317,1364,344,1369,357,1394,359,1415,363,1421,378,1426,389,1447,370,1463,352,1477,327,1453,287,1455,269,1434,263,1441,253,1452,253,1465,248,1484,218,1481,207,1458,196,1423,169,1423,170" shape="poly" @click.prevent="openModal('1eastAsiaTYPHOON')">
            <area v-if="currentActive === 'storm'" alt="2eastAsiaTYPHOON" title="2eastAsiaTYPHOON" coords="1541,222,1525,240,1527,259,1520,268,1517,275,1511,286,1508,300,1522,321,1536,300,1551,286,1562,270,1576,249,1567,238,1559,226" shape="poly" @click.prevent="openModal('2eastAsiaTYPHOON')">
            <area v-if="currentActive === 'storm'" alt="3eastAsiaTYPHOON" title="3eastAsiaTYPHOON" coords="1489,338,1476,358,1498,377,1519,356" shape="poly" @click.prevent="openModal('3eastAsiaTYPHOON')">
            <area v-if="currentActive === 'storm'" alt="4eastAsiaTYPHOON" title="4eastAsiaTYPHOON" coords="1490,388,1474,409,1482,437,1501,451,1511,462,1524,469,1543,453,1527,439,1522,402" shape="poly" @click.prevent="openModal('4eastAsiaTYPHOON')">
