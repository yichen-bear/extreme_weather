  ...
  <div class="leaderboard-page">
    ...
    <div class="leaderboard-container">
      <header class="page-header">
        ...
        <div class="mode-tabs">
          <button :class="{ active: currentMode === 'normal' }" @click="currentMode = 'normal'">
            一般模式
          </button>
          <button :class="{ active: currentMode === 'gesture' }" @click="currentMode = 'gesture'">
            手勢辨識
          </button>
        </div>
      </header>
          ...
          <div class="podium-avatar-wrapper">
            <div class="podium-avatar">
              <img
                ...
                :src="enforceHttps(topThree[1].avatar)"
                alt="avatar"
                class="lb-avatar-img"
              />
              <span v-else>{{
                topThree[1].username ? topThree[1].username.substring(0, 2).toUpperCase() : '??'
              }}</span>
            </div>
            <div class="char-badge" v-if="topThree[1].character">
              <img
                v-if="topThree[1].character.includes('.png')"
                :src="enforceHttps(topThree[1].character)"
                class="lb-avatar-img"
              />
              <div v-else-if="topThree[1].character === 'BALL'" class="badge-ball"></div>
            </div>
          </div>
          <div class="podium-info">
            <span class="podium-name">{{ topThree[1].username }}</span>
            <span class="podium-score">{{ topThree[1].score }} PTS</span>
          </div>
          ...
          <div class="podium-avatar-wrapper">
            <div class="podium-avatar">
              <img
                ...
                :src="enforceHttps(topThree[0].avatar)"
                alt="avatar"
                class="lb-avatar-img"
              />
              <span v-else>{{
                topThree[0].username ? topThree[0].username.substring(0, 2).toUpperCase() : '??'
              }}</span>
            </div>
            <div class="char-badge" v-if="topThree[0].character">
              <img
                v-if="topThree[0].character.includes('.png')"
                :src="enforceHttps(topThree[0].character)"
                class="lb-avatar-img"
              />
              <div v-else-if="topThree[0].character === 'BALL'" class="badge-ball"></div>
            </div>
          </div>
          <div class="podium-info">
            <span class="podium-name">{{ topThree[0].username }}</span>
            <span class="podium-score">{{ topThree[0].score }} PTS</span>
          </div>
          ...
          <div class="podium-avatar-wrapper">
            <div class="podium-avatar">
              <img
                ...
                :src="enforceHttps(topThree[2].avatar)"
                alt="avatar"
                class="lb-avatar-img"
              />
              <span v-else>{{
                topThree[2].username ? topThree[2].username.substring(0, 2).toUpperCase() : '??'
              }}</span>
            </div>
            <div class="char-badge" v-if="topThree[2].character">
              <img
                v-if="topThree[2].character.includes('.png')"
                :src="enforceHttps(topThree[2].character)"
                class="lb-avatar-img"
              />
              <div v-else-if="topThree[2].character === 'BALL'" class="badge-ball"></div>
            </div>
          </div>
          <div class="podium-info">
            <span class="podium-name">{{ topThree[2].username }}</span>
            <span class="podium-score">{{ topThree[2].score }} PTS</span>
          </div>
      ...
      <div class="lb-list" v-if="restOfList.length > 0">
        <div v-for="(entry, i) in restOfList" :key="i" class="lb-item" :style="{ '--delay': i }">
          ...
          <div class="lb-player-info">
            <div class="list-avatar-wrapper">
              ...
              <div class="char-badge list-badge" v-if="entry.character">
                <img
                  v-if="entry.character.includes('.png')"
                  :src="enforceHttps(entry.character)"
                  class="lb-avatar-img"
                />
                <div v-else-if="entry.character === 'BALL'" class="badge-ball"></div>
              </div>
            </div>
...
const fetchLeaderboard = async () => {
  try {
    ...
    // 💡 修改：根據 currentMode 加上 ?mode=... 參數
    const res = await axios.get(`${baseURL}/api/score/leaderboard?mode=${currentMode.value}`)
...
.step-1 .podium-rank,
.step-1 .podium-suffix {
  color: var(--c-gold);
}
.step-2 .podium-rank,
.step-2 .podium-suffix {
  color: var(--c-silver);
}
.step-3 .podium-rank,
.step-3 .podium-suffix {
  color: var(--c-bronze);
}
